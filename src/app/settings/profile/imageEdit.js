...
export default function ImageEdit({className, userPhoto, onFileUpload}) {
    ...
    return <div>
                <Image src={(imageFileUrl === "removed" ? backupProfileImage : imageFileUrl) || (userPhoto === "removed" ? null : userPhoto ) || backupProfileImage}
                    alt="Profile picture" width={200} height={200}
                    className={`border border-gray-400 rounded-full w-44 h-44 object-cover ${className}`}/>
                <EditButton uploadHandler={uploadHandler} removeHandler={removeHandler} className="relative -top-12 left-2"/>
            </div>
}
