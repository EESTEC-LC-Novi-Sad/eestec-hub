...
export default function ProfileForm({ userStr }) {
    ...
    return <div className="flex flex-col md:flex-row-reverse">
                    ...
                    <form action={handleSubmit} className="max-w-96">
                        ...
                        <input type="text" defaultValue={user?.location ?? ""} name="location" placeholder="eg. Novi Sad" className="px-2 border border-gray-300 rounded w-full"/>
                        <p className="mt-1 mb-4 text-sm text-gray-600">Your location will be visible on your public profile.</p>
                        <label htmlFor="birthday"><b>Birthday</b></label>
                        <input type="date" defaultValue={formatDateForInput(user?.birthDate) ?? ""} name="birthday" className="px-2 border border-gray-300 rounded w-full"/>
                        <p className="mt-1 mb-4 text-sm text-gray-600">Let the people know what generation you belong to! Your birthday will be visible on your public profile.</p>
                        ...
                        <p className="mt-1 mb-4 text-sm text-gray-600">Help other members find your socials, this is visible on your public profile page.</p>
                        <div className="flex mt-4 gap-2">
                            <Button type="submit" className="border-green-700 text-green-700">Save changes</Button>
                            <LinkButton href={`/profile/${user.username}`} className=" border-red-700 text-red-700">Cancel</LinkButton>
                        </div>
                    </form>
                </div>
}
