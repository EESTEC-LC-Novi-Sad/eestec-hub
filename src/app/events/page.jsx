...
export default async function EventsPage() {
    ...
    const events = (await getAllEvents()).sort((a, b) => b.startDate - a.startDate);
    ...
    const upcomingEventsByYear = eventYears.map(year => upcomingEvents.filter(e => e.startDate.getFullYear() === year));
    const pastEventsByYear = eventYears.map(year => pastEvents.filter(e => e.startDate.getFullYear() === year));

    return (
        <div className="flex justify-center">
            <div className="w-full md:w-7/12">
                <h1 className="text-2xl"><b>Upcoming events</b></h1>
                { !upcomingEvents || upcomingEvents.length === 0 
                    ? <p className="ml-4 text-gray-700 text-sm">No upcoming events</p>
                    : <div>{ upcomingEventsByYear.map((year, index) => {
                ...
                <Separator/>
                <h1 className="text-2xl"><b>Past events</b></h1>
                { !pastEvents || pastEvents.length === 0 
                    ? <p className="ml-4 text-gray-700 text-sm">No past events</p>
                    : <div>{ pastEventsByYear.map((year, index) => {
                        return <div key={`year-${index}`}>
                            <h1 className="text-center text-3xl mt-2"><b>{year[0].startDate.getFullYear()}</b></h1>
                            {year.map(mapEventToCard)}
                        </div>
