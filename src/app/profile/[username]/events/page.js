...
export default async function EventsByUsernamePage({ params }) {
	...
	const eventYears = [
		...new Set(userEvents.map((e) => e.startDate.getFullYear())),
	];
	const eventsByYear = eventYears.map((year) =>
		userEvents.filter((e) => e.startDate.getFullYear() === year),
	);
		...
		<div className="flex justify-center">
			<div className="w-full md:w-7/12">
				<h1 className="text-2xl ml-2">
					<b>Events you attended</b>
				</h1>
				{!userEvents || userEvents.length === 0 ? (
					<p>You have not attended any events</p>
				) : (
					<div>
						{eventsByYear.map((year, index) => {
							return (
								<div key={`year-${index}`}>
