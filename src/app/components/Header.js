...
function ModalDiv({ children, onClick, isOpen, className, isLeft }) {
	...
	return (
        ...
        w-72 absolute z-20 bg-white ${className}`}
		>
			{children}
		</div>
	);
...
export default function Header({ session }) {
	...
	const profileImage =
		(profileImageUri === "removed" ? null : profileImageUri) ??
		backupProfileImage;
	...
	const noHeaderRoutes = ["/login", "/signup", "/password-reset"];
	for (const route of noHeaderRoutes) {
		if (pathname.includes(route)) return <div />;
	}
		...
		<div>
			<div>
				...
				<ModalDiv isLeft={true} isOpen={isMenuOpen} className="top-0 py-4">
					...
					<ul className="mt-6">
						<MenuLink onClick={closeMenu} href="/">
							<HomeIcon className="mr-1" /> Home
						</MenuLink>
						<MenuLink onClick={closeMenu} href="/projects">
							<ProjectsIcon className="mr-1" /> Projects
						</MenuLink>
						<MenuLink onClick={closeMenu} href="/teams">
							<TeamsIcon className="mr-1" /> Teams
						</MenuLink>
						<MenuLink onClick={closeMenu} href="/events">
							<EventsIcon className="mr-1" /> Events
						</MenuLink>
				...
				<ModalDiv
					...
					className="top-0 right-0 py-4"
				>
					<Button onClick={closeProfileMenu} className="absolute right-4 top-4">
						<CloseIcon />
					</Button>
					<div className="flex items-center ml-3">
						...
						/>
						<div>
							<h2 className="ml-1 mr-6 -mb-2 px-1">
								<b>{session?.user?.username ?? "Unknown username"}</b>
							</h2>
					...
					<ul className="mt-6">
						...
						<Button onClick={() => signOut()} className="ml-4 mt-4">
							Logout
						</Button>
					</ul>
				</ModalDiv>
				<div className="flex justify-between items-center py-2 border border-solid border-gray-300">
					<div className="flex items-center">
						<Button className="mx-2" onClick={openMenu}>
							<MenuIcon />
						</Button>
						<h1>
							<b>{pageHeader}</b>
						...
						<ul className="flex items-center">
							<li className="mx-2">
								<LinkButton className="flex relative z-0" href="/notifications">
									...
									Notifications
									<div
										className={`${notificationsNum === 0 ? "hidden" : ""}
